...
module.exports = function loadModel({file, is_binary='auto'}, callback ) {
	var model = {};
	var vocab = [];
	var words;
	var size;
	...
	function init( file , is_binary ) {
		if( is_binary === 'auto' ) {
			...
			( is_binary ? readBinary : readTxt )( file );
		}
	}
	init( file , is_binary );
	...
	function readBinary( file ) {
		...
		fs.open( file, 'r', function( err, fd ) {
			...
			fs.read( fd, buffer, 0, 50, 0, function( err, bytesRead, buffer ) {
				...
				buffer = new Buffer.alloc( MAX_STRING + size * 4 );
